...
from ..serializers import DocumentXML1207Serializer, NotificationXML1207Serializer, ReplayDocumentXML1207Serializer
...
class ReplayDocument2Xml1207Converter():
    def __init__(self, document: BaseDocument, referred_document: BaseDocument, message_id: str, consumer: CoreOrganization, sign: str, attestation: str,
                 comment: str):
        self.message_id = message_id
        self.document: BaseDocument = document
        self.referred_document: BaseDocument = referred_document
        self.consumer: CoreOrganization = consumer
        self.sign = sign
    ...
    def create_xml(self):
        data = self.get_xml_data()
        return xml.serialize_to_string(ReplayDocumentXML1207Serializer, data, indent='  ')
    ...
    def get_xml_data(self):
        data = self.get_header_attrs()
        data['ReplayDocument'] = self.get_replay_document()
        data['Expansion'] = self.get_expansion()
        return data

    def get_replay_document(self):
        """ReplayDocument"""
        data = {'ReplayAddressee': self.get_consumer_org(),
                'Author': self.get_author(),
                'Confident': {'flag': '0'},
                'DocTransfer': self.get_doc_transfer(),
                'RegNumber': self.get_reg_number(),
                ...
                'Referred':
                    {
                        'RegNumber':
                            {
                                'regdate': self.referred_document.reg_date,
                                '.': self.document.reg_number
                            },
                        'idnumber': self.referred_document.pk,
                        'retype': '0',
                    },
                'type': '0',
                }
...
class Notification2Xml1207Converter():
    def __init__(self, message_id: str, producer: CoreOrganization, consumer: CoreOrganization, sign: str):
        self.message_id = message_id
    ...
    def create_xml(self):
        data = self.get_xml_data()
        return xml.serialize_to_string(NotificationXML1207Serializer, data, indent='  ')
